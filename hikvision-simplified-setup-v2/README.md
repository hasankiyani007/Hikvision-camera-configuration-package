...
| `disableCodecVerification: yes` | Disable check | Allow Hikvision proprietary header formats |
| `sourceOnDemandCloseAfter: 30s` | Idle timeout | Save camera bandwidth when no viewers |
...
├── bin/
│   ├── HikvisionCameraSetupCLI.exe   Main setup executable
│   ├── mediamtx.exe                  Open source RTSP streaming server
│   ├── mediamtx.yml                  Server configuration (auto edited)
│   └── data/                         Runtime camera database storage
├── shared/                           Core business logic libraries
│   ├── CameraSharedLib_Simplified    SADP network discovery protocol
│   ├── SimpleSoT                     Camera database management
│   ├── MediaMTXConfigManager         Automatic configuration generator
│   └── jsoncpp                       Embedded JSON parser
├── service/                          Windows background service
│   ├── MediaMTXService.cpp           Service source code
│   ├── MediaMTXService.h             Service header
│   ├── build.bat                     Service build script
│   ├── MediaMTXService.exe           Compiled service executable
│   ├── install_service.bat           Install as system service
│   └── uninstall_service.bat         Remove service
├── SimplifiedCameraSetup.cpp         Main CLI application source
